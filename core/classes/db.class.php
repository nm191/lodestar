<?php
  class DB{
    ...
    static private function getConnection(){
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=lodestar", $username, $password);
    ...
    static public function fetch($query, array $pdo_parameters_ar = array()){
      self::$conn = self::getConnection();
      $stmt = self::$conn->prepare($query);
      foreach($pdo_parameters_ar as $key => &$value){
        $stmt->bindParam($key, $value);
